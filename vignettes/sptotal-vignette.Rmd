...
We now have a data set where the whole population is known, `simdata`, and another one, `simobs`, where 75% of the population has been replaced by `NA`.  Next we show the sampled sites as solid circles, while the missing values are shown as open circles, and we use red again to show the sampled sites within the small area of 25 locations.
...
```{r}
require(rgeos); require(sp); require(rgdal)
...
head(pred_obj_area$Pred_df)
tail(pred_obj_area$Pred_df)
```
