...
## Examining results: `print()`, and `plot()`

Alternatively, you could take advantage of the pre-specified `print()` and `plot()` functions. Both take the output from `predict()`, `pred_obj` in this case, as a function argument.

```{r}
print(pred_obj)
```

We predict a total of 4817 units in this simulated region with 90% confidence bounds of (4779, 4856). The confidence interval is fairly small because we simulated data that were highly correlated, increasing precision in prediction for unobserved sites. We can see that the prediction of the total is close to the true value of 4834.326, and the true value is within the confidence interval.
