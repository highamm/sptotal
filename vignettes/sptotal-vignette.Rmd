...
The output of the `predict` function is too lengthy to fit on this page and can be a bit cumbersome to read anyway. Therefore, we would typically use the `FPBKoutput` function to create a nicer looking report. 

THIS NEEDS TO BE FIXED....NEED TO MAKE THE KRIGMAP PART

```{r}
## FPBKoutput(pred_obj, get_sampdetails = TRUE, get_variogram = TRUE, 
##  get_krigmap = TRUE, get_report = TRUE)
## fix this issue! get_report only works if all of the other options
## are set to TRUE
```

If we want to predict a quantity other than the population total, then we need to specify the column in our data set that has the appropriate prediction weights. For example, if we want to predict the total for the 25 sites in coloured in red, then we can use

```{r}
pred_obj2 <- predict(slmfit_out1, FPBKcol = "wts2")
```
...
##### Issue 1: Dealing with Coordinates

Our goal is to append the x and y-coordinates to the data frame with the survey data. Specifically, we want the _centroid_ coordinates for each site. We first want to convert the projection into Latitude/Longitude coordinates.
...
Finally, for most spatial prediction, we want to use a trans-mercator projection instead of latitude-longitude so that physical distance between two sites is accurately represented.

```{r}
xy <- LLtoUTM(mean(centroids$x), centroids$y, centroids$x)$xy
```

Finally, we add the trans-mercator coordinates to our data frame with the survey data. We can examine extract the survey data by

```{r}
data(AKmoose)
moose_df <- AKmoose@data
head(moose_df)
```
...
Now, the `moose_df` data frame is in a more workable form for the `sptotal` package. However, there is still an issue that may be somewhat common in real data sets, which we address next.
...
The first issue is that our `sp` object has `total` as a factor, which `R` treats as a categorical variable. `total` should be numeric, and, in fact, `surveyed` as the same issue. We first want to convert these two columns into numeric variables instead of factors.
...
* we have x and y coordinates in TM format and
...
predict(slmfit_out_moose)$FPBK_Prediction
```

Let's check to make sure the prediction is in the same ballpark as what we would get by simply scaling up the sample mean count in each stratum:

```{r}
testdf <- predict(slmfit_out_moose)$Pred_df
sum(testdf$total_pred[testdf$strat == "M"], na.rm = TRUE)
sum(testdf$total_pred[testdf$strat == "L"], na.rm = TRUE)
## it's the low stratum that's really off....... 
## need to consider this once a map is made to see if it can be explained
## spatially

low_strat <- subset(moose_df, strat == "L")
med_strat <- subset(moose_df, strat == "M")
mean(low_strat$total, na.rm = TRUE) * nrow(low_strat) +
  mean(med_strat$total, na.rm = TRUE) * nrow(med_strat)

## try doing this twice: once for each stratum.
slm_out_low <- slmfit(formula = total ~ 1, xcoordcol = 'x',
  ycoordcol = 'y', data = low_strat)
pred_low <- predict(slm_out_low)
slm_out_med <- slmfit(formula = total ~ 1, xcoordcol = 'x',
  ycoordcol = 'y', data = med_strat)
pred_med <- predict(slm_out_med)
pred_low$FPBK_Prediction + pred_med$FPBK_Prediction
```
