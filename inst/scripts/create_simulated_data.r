...
F1 <- as.factor(rep(1:3,
                    times = ceiling(length(eps)/3)))[seq_len(length(eps))]
F1 <- F1[order(runif(length(eps)))]
#create factor F2 variable
F2 <- as.factor(rep(1:5,
                    times = ceiling(length(eps)/5)))[seq_len(length(eps))]
...
simdata <- data.frame(x = xy$x, y = xy$y, X1 = X1,
                      X2 = X2, X3 = X3, X4 = X4,
                      X5 = X5, X6 = X6, X7 = X7,
                      F1 = F1, F2 = F2, Z = Z,
                      wts1 = wts1, wts2 = wts2)
